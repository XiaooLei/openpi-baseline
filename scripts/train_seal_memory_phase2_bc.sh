...
NUM_EVAL_BATCHES=${NUM_EVAL_BATCHES:-30}
NUM_HELDIN_EVAL_BATCHES=${NUM_HELDIN_EVAL_BATCHES:-10}
...
REQUIRED_PATHS=("$TOKENIZER_PATH" "$CHECKPOINT_PATH" "$NORM_STATS_PATH" "$EXPERT_DATASET_PATH")
if [[ "$CONFIG" == *phase2* ]]; then
  REQUIRED_PATHS+=("$PHASE2_DATASET_PATH" "$PHASE2_ADVANTAGE_PATH")
fi
